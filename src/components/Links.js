    ...
    return (
      <div>
        <div className='flex flex-col w-full mx-auto text-sm md:text-2xl lg:text-2xl sm:text-xl items-center text-center mt-8 px-5'>
          {links.map((link) => ( <a href={link.url} className='w-full bg-gray-300 rounded-lg active:bg-gray-700 mb-5 p-3 md:p-5 lg:p-5 font-bold hover:bg-gray-200' id={link.id}>{link.content}</a>))}
{/* 
